...
describe('App Component', () => {
    ...
    test('renders "Update General Ledger / Inventory" button', async () => {
        ...
        const updateButton = screen.getByRole('button', {
            name: /Update General Ledger \/ Inventory/i,
        });
    ...
    test('renders Product Name input field', async () => {
        ...
        const productNameInput =
            screen.getByPlaceholderText(/e.g., Organic Apples/i);
    ...
    test('renders Inventory List heading', async () => {
        ...
        render(<App />);
        await waitFor(() => expect(axios.get).toHaveBeenCalledTimes(1));
        const inventoryListHeading = screen.getByRole('heading', {
            name: /Inventory List/i,
        });
    ...
    test('initially shows "No products yet" message when fetch returns empty array', async () => {
        ...
        const noProductsMessage = await screen.findByText(
            /No products yet. Add some using the form above./i
        );
    ...
    test('displays products when fetch returns data', async () => {
        ...
        // Find all rows in the table. This includes the header row.
        const table = screen.getByRole('table');
        const rows = within(table).getAllByRole('row');

        let product2RowElement = null;
        // Iterate through rows to find the one containing 'Test Product 2'
        // Start from index 1 to skip header row if present and identified by getAllByRole
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            // queryByText returns the element or null, good for conditional checks
            if (within(row).queryByText('Test Product 2')) {
                product2RowElement = row;
                break;
            }
        }

        // Ensure the row containing "Test Product 2" was found
        expect(product2RowElement).not.toBeNull();

        // Now use 'within' for specific cells in that identified row
        // The if block was removed as expect(...).not.toBeNull() above ensures product2RowElement is not null.
        expect(within(product2RowElement).getByText('20')).toBeInTheDocument(); // Quantity
        expect(
            within(product2RowElement).getByText('200.00')
        ).toBeInTheDocument(); // Price
        expect(within(product2RowElement).getByText('21')).toBeInTheDocument(); // VAT
        expect(
            within(product2RowElement).getByText('240.00')
        ).toBeInTheDocument(); // Future Price
        expect(within(product2RowElement).getByText('22')).toBeInTheDocument(); // Future VAT
    });
});
